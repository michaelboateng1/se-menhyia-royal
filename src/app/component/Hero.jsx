...
const Hero = () => {
  ...
  return (
    <section className="w-full h-[90vh] flex items-center justify-center relative overflow-hidden border-b-8 border-[#da9408] px-4 md:px-12">
      <div className="w-full h-full relative flex items-center justify-center z-30 pointer-events-none">
        <FaArrowAltCircleLeft className="absolute top-1/2 -translate-y-1/2 left-0 md:left-10 text-4xl md:text-5xl text-white/80 hover:text-white cursor-pointer z-30 select-none pointer-events-auto" onClick={() => setCurrent((prev) => (prev - 1 + images.length) % images.length)} />
        <FaArrowAltCircleRight className="absolute top-1/2 -translate-y-1/2 right-0 md:right-10 text-4xl md:text-5xl text-white/80 hover:text-white cursor-pointer z-30 select-none pointer-events-auto" onClick={() => setCurrent((prev) => (prev + 1) % images.length)} />
      </div>
      ...
      <h1 className="school-name absolute md:top-[20%] lg:top-[32%] left-1/2 -translate-x-1/2 text-3xl md:text-5xl font-bold text-white drop-shadow-lg md:space-y-2 lg:text-nowrap z-20 text-center px-2 md:px-0">
        <span className="text-[#da9408]">Se-Manhyia Royal</span> International School
      </h1>

      {
        images.map((slide, i) => (
          <div key={i} className={`absolute bottom-24 md:bottom-30 lg:bottom-63 w-full text-center text-white px-4 transition-opacity duration-1000 ${i === current ? "opacity-100 z-20" : "opacity-0 z-0"}`}>
            <p className="school-name text-xl md:text-3xl font-semibold drop-shadow-lg mb-4 md:mb-6">{slide.caption}</p>
          </div>
        ))[current]
      }

      <Link href="/admissions" className="text-xl absolute bottom-6 md:bottom-1110 lg:bottom-45 left-1/2 -translate-x-1/2 bg-[#da9408] hover:bg-[#b27c07] text-white font-semibold px-6 py-3 rounded-full shadow-lg transition-colors duration-300 z-20 pointer-events-auto">
        Apply Now
      </Link>
    </section>
  );
